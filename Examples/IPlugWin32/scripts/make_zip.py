...
def  main():
  ...
  installer = "\installer\IPlugWin32 Installer.exe"
   
  if demo:
    installer = "\installer\IPlugWin32 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugWin32 manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugWin32-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugWin32-v" + FULLVERSIONSTR + "-win-demo.zip"
