...
void IPlugReaperPlugin::OnEmbeddedUIMouseLeft(int x, int y, bool down)
{
  ...
  mod.L = true;
  if(down)
    mMiniEditor.GetUI()->OnMouseDown(x, y, mod);
  else
    mMiniEditor.GetUI()->OnMouseUp(x, y, mod);
}
...
void IPlugReaperPlugin::DrawEmbeddedUI(LICE_IBitmap* pBitmap, int mouseX, int mouseY, bool leftMouseDown, bool rightMouseDown)
{
